...
- [x] **Email регистрация**: создать аккаунт → получить JWT → сессия работает
- [x] **Email вход**: повторный логин → токены обновляются
- [x] **Refresh token**: дождаться expiry access token → автоматический refresh без logout
- [x] **Telegram Login Widget**: авторизация через Telegram → профиль создан ⚠️ *требует `NEXT_PUBLIC_TELEGRAM_BOT_NAME` в .env + docker-compose + Dockerfile*
- [x] **Google OAuth**: авторизация через Google → профиль создан/связан ⚠️ *требует `GOOGLE_CLIENT_ID` + `GOOGLE_CLIENT_SECRET` в .env*
- [x] **Guest mode**: анонимная сессия → DNA сохраняется в localStorage/cookie
- [x] **Rate limiting**: 6 неверных паролей подряд → 429 Too Many Requests
- [x] **Logout**: токены инвалидированы → повторный запрос с тем же токеном → 401

### 15.3 Эмоциональный онбординг → Literary DNA

> **Статус кода**: баги исправлены (selectinload в `get_recently_finished`, book eager-load в `update_shelf_entry`). Осталось только ручное тестирование на живом стенде.
...
- [ ] DNA меняется после добавления первой книги с оценкой + рецензией (только review запускает AI-обновление через `_bg_dna_update_from_review`; рейтинг без рецензии перезапускает recompute, но не изменит метрики у нового пользователя без истории)

### 15.4 Каталог книг
- [x] Отобразить список книг → карточки с обложкой, emotional tags, рейтингом
- [x] **Semantic search**: запрос "меланхоличный Петербург" → релевантные книги (не keyword match)
- [x] **AI Prompt search**: "хочу как Достоевский, но в космосе" → осмысленные результаты
- [x] Открыть книгу → страница с описанием, why recommended, цитаты друзей
- [x] Добавить книгу в статус "reading" → появляется в библиотеке профиля
- [x] Сменить статус → "read" → добавить оценку + рецензию → DNA обновилась

### 15.5 Рекомендации
- [x] **По Literary DNA**: `/recommendations` → "Обновить DNA" → pgvector подбирает книги по эмбеддингам
- [x] **По настроению**: chip "Напряжённое" (mood=тревожно) → `/recommendations/by-mood` endpoint
- [x] **Collaborative filtering**: "Обновить похожих" → `/recommendations/refresh/collaborative`
- [x] **AI объяснение**: кнопка "Почему эта книга тебе?" → `POST /{rec_id}/explain` → AI текст
- [x] **AI Prompt**: `/discover` → свободный текстовый запрос → AI возвращает 3–5 книг с объяснением

### 15.6 Читательский дневник
- [x] Создать запись дневника (текст + настроение + цитата)
- [x] Убедиться что AI проанализировал запись → DNA обновилась (emotion weights изменились)
- [x] Редактировать запись → сохраняется
- [x] Удалить запись → исчезает из списка, DNA пересчитывается
